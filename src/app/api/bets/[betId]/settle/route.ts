...
import { BetIdSchema, BetSettleRequestSchema, IdempotencyKeySchema } from '@/lib/bets/schemas';
...
export async function POST(request: Request, context: { params: Promise<{ betId: string }> }) {
  ...
  const body = BetSettleRequestSchema.safeParse(await request.json().catch(() => null));
  if (!body.success) { return codedErrorResponse(body.error.issues[0]?.message ?? 'Invalid settlement', 'VALIDATION_ERROR', 400, body.error.issues[0]?.path.join('.')); }

  try {
    const result = await settleBet(createServiceRoleClient(), user.id, betId, body.data, key.data);
