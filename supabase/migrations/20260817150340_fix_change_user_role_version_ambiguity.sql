...
begin
  ...
  if not exists (
    ...
    where users.id = p_actor_user_id and users.role = 'admin'
    for key share
  ) then
  ...
  if v_target.role_version <> p_expected_role_version then
    raise exception 'ROLE_VERSION_CONFLICT' using errcode = '40001';
  end if;

  update public.users
  set role = p_role, role_version = public.users.role_version + 1
  where users.id = p_target_user_id
  returning users.id, users.email, users.role, users.role_version, users.created_at
  into id, email, role, role_version, created_at;

  insert into public.audit_logs (entity_type, entity_id, action, actor_id)
  values ('user', p_target_user_id, 'role_changed', p_actor_user_id);
  return next;
end;
$$;

revoke all on function public.change_user_role(uuid, uuid, text, bigint)
