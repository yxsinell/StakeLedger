...
begin
  ...
  insert into public.transaction_idempotencies (
    user_id, idempotency_key, request_payload, transaction_id, response_payload
  ) values (
    v_user_id, p_idempotency_key, v_payload, v_debit_transaction_id, '{}'::jsonb
  ...
  from public.banks where id = p_source_bank_id;

  if not found or v_source_user_id <> v_user_id then
    raise exception 'BANK_NOT_FOUND' using errcode = 'P0001';
  end if;

  select user_id, currency into v_destination_user_id, v_destination_currency
  from public.banks where id = p_destination_bank_id;

  if not found or v_destination_user_id <> v_user_id then
    raise exception 'BANK_NOT_FOUND' using errcode = 'P0001';
  end if;
  ...
  if p_source_bank_id < p_destination_bank_id then
    ...
    where bank_id = p_source_bank_id and pocket_type = 'cash'
    for update;
    ...
    from public.bank_pockets
    where bank_id = p_destination_bank_id and pocket_type = 'cash'
    for update;
  ...
  insert into public.transactions (
    id, bank_id, pocket_type, type, amount, transfer_id, idempotency_key
  ) values (
    v_debit_transaction_id, p_source_bank_id, 'cash', 'transfer_debit',
    p_amount, v_transfer_id, p_idempotency_key
  );

  insert into public.transactions (
    id, bank_id, pocket_type, type, amount, transfer_id, related_transaction_id
  ) values (
    v_credit_transaction_id, p_destination_bank_id, 'cash', 'transfer_credit',
    p_amount, v_transfer_id, v_debit_transaction_id
  );
...
drop function public.record_cash_transfer(uuid, uuid, numeric, uuid);

revoke all on function public.record_cash_transfer(uuid, uuid, uuid, numeric, uuid)
  from public, anon, authenticated;
grant execute on function public.record_cash_transfer(uuid, uuid, uuid, numeric, uuid)
  to service_role;
