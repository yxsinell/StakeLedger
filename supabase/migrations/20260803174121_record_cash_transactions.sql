create table public.transaction_idempotencies (
  user_id uuid not null references public.users (id) on delete cascade,
  idempotency_key uuid not null,
  request_payload jsonb not null,
  transaction_id uuid not null unique references public.transactions (id) on delete restrict deferrable initially deferred,
  response_payload jsonb not null,
  created_at timestamptz not null default now(),
  primary key (user_id, idempotency_key),
  constraint transaction_idempotencies_request_payload_object
    check (jsonb_typeof(request_payload) = 'object'),
  constraint transaction_idempotencies_response_payload_object
    check (jsonb_typeof(response_payload) = 'object')
);

alter table public.transaction_idempotencies enable row level security;

revoke all on table public.transaction_idempotencies from public, anon, authenticated;

alter table public.transactions
  add constraint transactions_cash_deposit_withdrawal_check check (
    type not in ('deposit', 'withdraw')
    or idempotency_key is null
    or (
      pocket_type = 'cash'
      and method in ('bank_transfer', 'card', 'cash')
    )
  );
...
revoke all on function public.record_cash_transaction(uuid, text, numeric, text, uuid)
  from public, anon, service_role;
grant execute on function public.record_cash_transaction(uuid, text, numeric, text, uuid)
  to authenticated;
